...
--
-- Indeksy dla tabeli `old_addres-- phpMyAdmin SQL Dump
-- version 5.2.0
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1
-- Czas generowania: 01 Gru 2023, 01:18
-- Wersja serwera: 10.4.27-MariaDB
-- Wersja PHP: 8.0.25
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddOrder` (IN `p_user_id` INT, IN `p_book_id` INT, IN `p_quantity` INT)   BEGIN
    -- Dodaj zamówienie
    ...
    SELECT @order_id, p_book_id, p_quantity, price, p_quantity * price FROM books WHERE id = p_book_id;

    -- Oblicz łączną cenę zamówienia
    UPDATE orders SET price = p_quantity * (SELECT price FROM books WHERE id = p_book_id) WHERE id = @order_id;
...
(2, 'Piotrek', 'Piotrek', 'Jakis', 'example@oop.pl', '$2y$10$1ZO.u.Cf/YsPaOs/1X09SO0PakylvP5wNktwJ4maBWmbrM6abLqNO'),
(4, 'admin', 'admin', 'admin', 'admin@bookstore.com', '$2y$10$jv.kHF1EBYUKfw8Hcl919.uv0E.0h/mvsCLEwa3eejkL3YgTVp0yq'),
(5, 'grosik', 'barte', 'grosicki', 'grosik@ex.pl', '$2y$10$.kuN0oqaiXIE68WVkoWYJeoglpMmCRrdNsHOrDqdroRphYK6KQgxq');
